...
<link href="https://cdn.jsdelivr.net/npm/summernote@0.8.18/dist/summernote.min.css" rel="stylesheet">
    ...
    <body>
        <div class="container-fluid border border-light shadow p-4 rounded">
            <h3 class=" text-primary mb-4 text-center" style='font-weight:bold; font-size:20px;'>ALL SUBSUBCATEGORY LIST</h3>
                ...
                <tbody>
                    @foreach ($subsubcategories as $subsubcategory)
                        ...
                        <tr style="border: 0.1px solid #d6d2d2;">
                            ...
                            <td style="border: 0.1px solid #d6d2d2;">{{ $subsubcategory->category->name }}</td>
                            {{-- <td style="border: 0.1px solid #d6d2d2;">{{ $subsubcategory->subcategory->name }}</td> --}}
                            <td style="border: 0.1px solid #d6d2d2;">
                                @isset($subsubcategory->subcategory)
                                    {{ $subsubcategory->subcategory->name }}

                                @endisset
                            </td>
                            {{-- <td style="border: 0.1px solid #d6d2d2;">{{ optional($subsubcategory->subcategory)->name ?? 'No Subcategory' }}</td> --}}

                            <td style="border: 0.1px solid #d6d2d2;">{{ $subsubcategory->name }}</td>
                            <td style="border: 0.1px solid #d6d2d2;">
                                <img src="{{ asset($subsubcategory->image) }}" alt="{{ $subsubcategory->name }}"
                                    style="width: 60px; height: 60px;">
                            </td>
                            <td style="border: 0.1px solid #d6d2d2;">{{ $subsubcategory->topbar_heading}}</td>
                            <td style="border: 0.1px solid #d6d2d2; max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{!! $subsubcategory->topbar_description !!}</td>
                            {{-- <td style="border: 0.1px solid #d6d2d2;max-width: 80px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ $subsubcategory->bottom_description }}</td> --}}
                            {{-- <td style="border: 0.1px solid #d6d2d2;">{{ $subsubcategory->meta_title }}</td> --}}

                            <td style="border: 0.1px solid #d6d2d2;">
                                <form action="{{ route('subsubcategory.destroy', $subsubcategory->id) }}" method="post">
                                    @csrf
                                    @method('DELETE')
                                    <a data-modal-target="crud-modal-2_{{ $subsubcategory->id }}"
                                        data-modal-toggle="crud-modal-2_{{ $subsubcategory->id }}"
            ...
            {{-- <a href="" class="btn btn-success mb-3 ">Add subsubcategory</a> --}}
    ...
    <div id="crud-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full" style="max-width: 40rem;">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" action="{{ route('subsubcategory.store')}}" method="POST"
                    ...
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        ...
                        <div class="col-span-1">
                            <label for="topbar_heading"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Heading</label>
                            <input type="text" name="topbar_heading" id="topbar_heading"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Type heading">
                                @if ($errors->has("topbar_heading"))
                                <p class="text-danger">{{ $errors->first("topbar_heading") }}</p>
                            @endif
                        </div>
                        <div class="col-span-1">
    ...
    @foreach ($subsubcategories as $subsubcategory)
        <div id="crud-modal-2_{{ $subsubcategory->id }}" tabindex="-1" aria-hidden="true"
            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-md max-h-full" style="max-width: 35rem;">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" action="{{ route('subsubcategory.update', $subsubcategory->id) }}" method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2 sm:col-span-1">
                                <label for="subsubcategory" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Category_Name</label>
                                <select id="subsubcategory" name="category_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required="">
                                    <option value="" selected="">Select Category</option>
                                    @foreach ($categories as $category)
                            ...
                            <div class="col-span-1">
                                <label for="name"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                                <input type="text" name="name" id="name"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ $subsubcategory->name }}"
                                    placeholder="Type product name" required="">
                            </div>
                            <div class="col-span-1">
                                <label for="heading"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Heading</label>
                                <input type="text" name="topbar_heading" id="heading"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ $subsubcategory->topbar_heading }}"
                                    placeholder="Type heading">
                            </div>
                            <div class="col-span-1">
                                <label for="meta_title"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Meta_Title</label>
                                <input type="text" name="meta_title" id="meta_title"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="{{ $subsubcategory->meta_title }}"
                                    placeholder="Type meta_title" >
                            </div>
                            <div class="col-span-2">
                                <label for="topbar_description"
                                ...
                                <textarea id="summernote" rows="4" name="topbar_description"
                                    class="summernote block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    placeholder="Write topbar_description">{!! $subsubcategory->topbar_description !!}</textarea>
                            </div>
                            <div class="col-span-2">
                                <label for="bottom_description"
                                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Bottom_Description</label>
                                <textarea id="summernote" rows="4" name="bottom_description"
                                    class="summernote block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    placeholder="Write bottom_description">{!! $subsubcategory->bottom_description !!}</textarea>
                            </div>
                            <div class="col-span-2">
                                <label for="image"
                            ...
                            <div>
                                <div class="col-span-2" id='currentImageDiv'>
                                    ...
                                    <img class="border border-1 rounded" style="width:80px;"
                                        src="{{ asset($subsubcategory->image) }}" alt="{{ $subsubcategory->name }}">
                                </div>
     ...
     $('#category_id').on('change', function () {
         ...
         $.ajax({
             ...
             success: function (response) {
                 var options = '<option value="" selected="">Select Sub_Category</option>';
